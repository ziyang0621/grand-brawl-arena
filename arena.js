...
let shake=0,calloutTime=2,cinematicTime=0;
...
function events(){const batch=world.events.splice(0);if(netRole==='host')netEvents.push(...batch);for(const e of batch){
  ...
  if(e.type==='skill'){const color=e.level===3?'#ffd84f':e.level===2?'#8ff2ff':'#7ef3e0';ringEffect(e,color,e.radius||3.4,.5+e.level*.12);particles(e,color,18+e.level*12);announce(e.level===3?'三级奥义 · 港湾终结斩！':e.level===2?'二级必杀 · 强化旋风斩！':'一级必杀 · 旋风斩！');shake=Math.max(shake,e.level*.12);cinematicTime=e.level===3?.9:cinematicTime;tone(420+e.level*80,.22+e.level*.08,'triangle');}
  ...
  if(e.type==='waveStart'){announce('巨浪来袭！跳上绿色甲板！');shake=.28;tone(65,.45,'sawtooth');}
...
function updateVisuals(dt){
  world.fighters.forEach((p,i)=>{
    ...
    if(p.carrying){m.arms[0].rotation.x=-2.65;m.arms[1].rotation.x=-2.65;m.arms[0].rotation.z=.18;m.arms[1].rotation.z=-.18;}
  ...
  const status=local.carrying?'举着容器 · J前投 / U高投':local.knocked>0?'倒地中':local.blocking?'防御中':local.climbing?'爬楼梯中':local.poisonTime>0?`中毒 ${local.poisonTime.toFixed(1)}s · 禁止闪避`:local.virusTime>0?`病毒感染 ${local.virusTime.toFixed(1)}s · 禁止闪避`:local.slowTime>0?`冰冻减速 ${local.slowTime.toFixed(1)}s · 禁止闪避`:local.burnTime>0?`炸伤冒烟 ${local.burnTime.toFixed(1)}s`:local.weapon==='sword'?`强化刀 ${local.attackBoostTime.toFixed(1)}s`:local.attackBoostTime>0?`啤酒力量 ${local.attackBoostTime.toFixed(1)}s`:'';$('movement').textContent=`${local.climbing?'楼梯':local.grounded?(local.support==='ground'?'地面':'已站稳绿色甲板'):'空中'} · 高度 ${local.y.toFixed(2)} · 二段跳 ${local.jumps}/2${status?' · '+status:''}`;
  const focus=world.fighters[localPlayerId]||world.fighters[0],focusX=Math.max(-5,Math.min(5,focus.x*.45)),focusZ=Math.max(-2,Math.min(2,focus.z*.24));cinematicTime=Math.max(0,cinematicTime-dt);camera.position.x=focusX;camera.position.z=(cinematicTime>0?20:25)+focusZ*.18;camera.lookAt(focusX,cinematicTime>0?1.5:1,focusZ);if(shake>0){shake=Math.max(0,shake-dt);camera.position.x+= (Math.random()-.5)*shake*.8;}
}
