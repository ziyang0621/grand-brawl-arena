...
function applyAction(p,code,data={}){const input=data.input||serialInput();if(code==='Space')jump(p);if(code==='KeyJ')attack(world,p,input);if(code==='KeyU')heavy(world,p,input);if(code==='KeyI')grab(world,p,input);if(code==='KeyK')bomb(world,p);if(code==='KeyL')skill(world,p,data.level||1);if(code==='ShiftLeft'||code==='ShiftRight')dodge(p);}
function bindHostConnection(conn){
  if(netConn){conn.on('open',()=>{conn.send({t:'reject',reason:'房间已有玩家'});conn.close();});return;}
  netConn=conn;conn.on('data',data=>{if(!data||typeof data!=='object')return;if(data.t==='hello'){world.online=true;world.training=false;started=true;networkStatus('朋友已加入 · 3D 联机');$('room3dLabel').textContent='房间 '+netRoom;conn.send({t:'welcome',you:1,room:netRoom});sendSnapshot();}else if(data.t==='input'&&data.input)world.remoteInput={x:Number(data.input.x)||0,z:Number(data.input.z)||0,guard:Boolean(data.input.guard)};else if(data.t==='action'&&data.code)applyAction(world.fighters[1],data.code,data);});
...
function networkAction(code,data){if(netRole==='guest'){sendNet({t:'action',code,...data});started=true;return false;}return true;}
...
bindNetworkUi();
function containerModel(){const g=new THREE.Group(),crate=new THREE.Group(),barrel=new THREE.Group(),chest=new THREE.Group();g.add(crate,barrel,chest);box(.95,.95,.95,'#ae7d4c',crate,0,.48,0);for(const y of [.1,.85])box(1.02,.11,1.02,'#5e5046',crate,0,y,0);const slat=box(1.13,.12,.06,'#e1b479',crate,0,.5,.5);slat.rotation.z=.7;cylinder(.43,.48,.95,'#9a653e',barrel,0,.48,0);for(const y of [.16,.78])cylinder(.48,.48,.09,'#3d4650',barrel,0,y,0);box(1.1,.72,.82,'#8c5f35',chest,0,.38,0);const lid=box(1.14,.3,.86,'#c38a43',chest,0,.88,0);lid.rotation.z=-.08;box(.2,.26,.08,'#f0c85c',chest,0,.51,.45);g.userData={crate,barrel,chest,kind:''};return g;}
...
const bombModels=new Map(),cloudModels=new Map(),propModels=new Map(),cannonModels=new Map(),effects=[],pickupModels=[];
const waveMesh=new THREE.Mesh(new THREE.BoxGeometry(34,1.2,19),new THREE.MeshBasicMaterial({color:'#77dded',transparent:true,opacity:0,depthWrite:false}));waveMesh.position.y=.2;waveMesh.visible=false;scene.add(waveMesh);
...
let calloutTime=2,cameraHalfH=10;
...
function events(){const batch=world.events.splice(0);if(netRole==='host')netEvents.push(...batch);for(const e of batch){
  if(e.type==='hit'){particles(e,'#ffe898');const tag=label('-'+e.damage,'#ffecab',65);tag.position.set(e.x,e.y+1,e.z);tag.scale.set(1.4,.5,1);scene.add(tag);effects.push({m:tag,life:.6,max:.6,v:new THREE.Vector3(0,2,0),sprite:true});tone(e.damage>=18?85:120,.1,'sawtooth');}
  ...
  if(e.type==='slash'||e.type==='shieldBash'){
    const color=e.attackType==='shieldBash'?'#72dbff':e.attackType==='slam'?'#ffbd61':e.weapon==='sword'?'#ffd648':e.id===0?'#fff1a6':'#a7f4ff';const m=new THREE.Mesh(new THREE.RingGeometry(e.attackType==='slam'?1.8:1.4,e.attackType==='slam'?2.8:2.4,24,1,-1.1,2.2),new THREE.MeshBasicMaterial({color,transparent:true,opacity:.8,side:THREE.DoubleSide,depthWrite:false}));m.rotation.set(-Math.PI/2,0,-Math.atan2(e.fz,e.fx));m.position.set(e.x,e.y,e.z);scene.add(m);effects.push({m,life:.2,max:.2});tone(e.attackType==='slam'?150:310,.06,'triangle');if(e.id===0)announce(e.attackType==='slam'?'坠落重击！':e.attackType==='air'?'空中斩！':e.attackType==='dash'?'冲刺斩！':e.attackType==='upper'?'上挑斩！':['一斩！','二连斩！','重斩！'][e.combo]);
  }
  if(e.type==='heavy'){ringEffect(e,'#ffb45c',2.2,.22);particles(e,'#ffe19a',12);announce('重击！');tone(180,.12,'sawtooth');}
  if(e.type==='grab'){announce('擒抱！按 J 前投 / U 高投；被抓者连按攻击挣脱');tone(260,.09,'square');}
  if(e.type==='throwHit'){ringEffect(e,'#ffcf72',1.5,.28);announce('抓取投摔！');tone(100,.18,'sawtooth');}
  if(e.type==='grabEscape'){ringEffect(e,'#8ff3e7',1.4,.24);announce('挣脱擒抱！');tone(360,.1,'triangle');}
  if(e.type==='explosion'){particles(e,e.kind==='poison'?'#83f09a':e.kind==='slow'?'#8ab8ff':'#ffab4c',32);ringEffect(e,e.kind==='poison'?'#8dffac':e.kind==='slow'?'#8ab8ff':'#ffdf76',3);tone(70,.24,'sawtooth',.06);}
  ...
  if(e.type==='skill'){const color=e.level===3?'#ffd84f':e.level===2?'#8ff2ff':'#7ef3e0';ringEffect(e,color,e.radius||3.4,.5+e.level*.12);particles(e,color,18+e.level*12);announce(e.level===3?'三级奥义 · 港湾终结斩！':e.level===2?'二级必杀 · 强化旋风斩！':'一级必杀 · 旋风斩！');tone(420+e.level*80,.22+e.level*.08,'triangle');}
  ...
  if(e.type==='cannon'){announce('港口炮击！注意两侧');tone(80,.28,'sawtooth');}
  if(e.type==='waveWarning'){announce(`巨浪将从${e.dir>0?'左':'右'}侧袭来！`);tone(140,.35,'triangle');}
  if(e.type==='waveStart'){announce('巨浪来袭！跳上绿色甲板！');tone(65,.45,'sawtooth');}
  if(e.type==='guard'){ringEffect(e,'#8bd8ff',1.5,.18);announce('挡住了！');tone(220,.08,'triangle');}
  if(e.type==='parry'){ringEffect(e,'#fff1a0',1.7,.28);announce('完美反击！');tone(760,.16,'triangle');}
...
function action(code){const p=world.fighters[localPlayerId];if(world.ended)return;if([...moveCodes,'Space','KeyJ','KeyU','KeyI','KeyK','KeyL','ShiftLeft','ShiftRight'].includes(code))started=true;const data={input:serialInput(),level:code==='KeyL'?(keys.has('KeyR')?3:keys.has('ShiftLeft')||keys.has('ShiftRight')?2:1):1};if(!networkAction(code,data))return;applyAction(p,code,data);}
...
function updateVisuals(dt){
  world.fighters.forEach((p,i)=>{
    ...
    if(p.attackTime>0){const total=['heavy','slam','upper','shieldBash'].includes(p.attackType)?.5:p.attackType==='grab'?.42:p.attackType==='dash'?.38:.34,swing=Math.max(0,Math.min(1,1-p.attackTime/total)),arc=Math.sin(swing*Math.PI);if(p.attackType==='grab'){m.arms[0].rotation.x=-.45-arc*1.15;m.arms[1].rotation.x=-.45-arc*1.15;m.arms[0].rotation.z=.25;m.arms[1].rotation.z=-.25;}else if(p.attackType==='slam'){m.arms[1].rotation.x=-2.3+arc*3.4;m.arms[1].rotation.z=-.6+arc*1.4;}else if(p.attackType==='shieldBash'){m.arms[0].rotation.x=-1.3+arc*.7;m.arms[0].rotation.z=.35;m.arms[1].rotation.x=-1.3+arc*.7;m.arms[1].rotation.z=-.35;}else{const comboSwing=p.attackType==='light'?(p.combo===1?2.8:2.6):2.6;m.arms[1].rotation.x=-1.5+arc*(['heavy','upper'].includes(p.attackType)?3.1:comboSwing);m.arms[1].rotation.z=-.8+arc*(['heavy','upper'].includes(p.attackType)?2.15:1.8);}}
    ...
    if(p.carrying||p.grabbedTarget!==null){m.arms[0].rotation.x=-2.65;m.arms[1].rotation.x=-2.65;m.arms[0].rotation.z=.18;m.arms[1].rotation.z=-.18;}
    if(p.grabbedBy!==null){m.arms[0].rotation.x=-1.9;m.arms[1].rotation.x=-1.9;m.legs.forEach((leg,j)=>leg.rotation.x=j===0?-.65:.65);m.body.rotation.z=.42;}
    ...
    if(p.grabbedBy===null){const runLean=walking?-.1:0,jumpLean=!p.grounded?-.08:0,landSquash=(p.landTime||0)>0?1-(p.landTime/.2)*.12:1;m.body.rotation.z=p.knocked>0?-.9:p.hurtTime>0?-.2:p.stun>0?-.24:runLean+(!p.grounded?jumpLean:0);m.body.scale.set(1,p.hurtTime>0?1.04+Math.sin(world.tick*.8)*.025:landSquash,1);if(!p.grounded){m.arms[0].rotation.x=-1.15;m.arms[1].rotation.x=-1.3;m.legs.forEach((leg,j)=>leg.rotation.x=j===0?-.7:.3);}}
    m.body.visible=!(p.invuln>0&&Math.floor(world.tick/6)%2===0);
  ...
  crateModels.forEach((m,i)=>{const c=world.crates[i];m.visible=c.hp>0||c.falling||c.heldBy!==null;setContainerKind(m,c.kind);m.position.set(c.x,c.y||0,c.z);m.rotation.z=c.falling?Math.sin(world.tick*.12+i):0;});
  ...
  for(const c of world.cannonballs||[]){let g=cannonModels.get(c.id);if(!g){g=sphere(.34,'#202a34',scene,0,0,0);cannonModels.set(c.id,g);}g.position.set(c.x,c.y,c.z);g.rotation.x+=dt*7;}for(const [id,g] of cannonModels)if(!(world.cannonballs||[]).some(c=>c.id===id)){scene.remove(g);cannonModels.delete(id);}
  waveMesh.visible=(world.waveTime||0)>0;if(waveMesh.visible){waveMesh.material.opacity=.22+Math.sin(world.tick*.2)*.08;waveMesh.position.x=-(world.waveDir||1)*10+(2.2-world.waveTime)*(world.waveDir||1)*10;}
  ...
  const [redFighter,blueFighter]=world.fighters;const local=world.fighters[localPlayerId];$('livesMode').textContent=world.stock?'三命模式：开':'三命模式：关';$('playerHP').value=redFighter.hp;$('rivalHP').value=blueFighter.hp;const redLives=world.stock?` · ${redFighter.lives}命`:'';const blueLives=world.stock?` · ${blueFighter.lives}命`:'';$('playerInfo').textContent=`${redFighter.hp} / 100${redLives} · ${redFighter.item?ITEM_NAMES[redFighter.item]:'空手'}`;$('rivalInfo').textContent=`${blueFighter.hp} / 100${blueLives} · ${blueFighter.item?ITEM_NAMES[blueFighter.item]:'空手'}`;updateStatusBadge('playerStatus',redFighter);updateStatusBadge('rivalStatus',blueFighter);$('time').textContent=Math.ceil(world.time);document.querySelector('.red span').textContent=localPlayerId===0?'YOU':'HOST';document.querySelector('.blue span').textContent=localPlayerId===1?'YOU':'CPU';
  $('bombLabel').textContent=local.item?(local.item==='sword'?'装备强化木刀':`使用${ITEM_NAMES[local.item]}`):local.carrying?'正举着容器':'先打碎箱子';$('skillLabel').textContent=local.energy<1?'能量不足':local.skillCD>0?`${local.skillCD.toFixed(1)}s`:local.energy>=3?'L / Shift+L / R+L':local.energy>=2?'L / Shift+L':'一级必杀';if($('energy')){$('energy').value=local.energy;$('energyLabel').textContent=`能量 ${local.energy.toFixed(1)} / ${local.energyMax}`;}
  const status=local.grabbedBy!==null?'被擒抱 · 连按 J/U/I 挣脱':local.grabbedTarget!==null?'已抱住对手 · J 前投 / U 高投':local.carrying?'举着容器 · J前投 / U高投':local.knocked>0?'倒地中':local.blocking?'防御中':local.climbing?'爬楼梯中':local.poisonTime>0?`中毒 ${local.poisonTime.toFixed(1)}s · 禁止闪避`:local.virusTime>0?`病毒感染 ${local.virusTime.toFixed(1)}s · 禁止闪避`:local.slowTime>0?`冰冻减速 ${local.slowTime.toFixed(1)}s · 禁止闪避`:local.burnTime>0?`炸伤冒烟 ${local.burnTime.toFixed(1)}s`:local.weapon==='sword'?`强化刀 ${local.attackBoostTime.toFixed(1)}s`:local.attackBoostTime>0?`啤酒力量 ${local.attackBoostTime.toFixed(1)}s`:'';$('movement').textContent=`${local.climbing?'楼梯':local.grounded?(local.support==='ground'?'地面':'已站稳绿色甲板'):'空中'} · 高度 ${local.y.toFixed(2)} · 二段跳 ${local.jumps}/2${status?' · '+status:''}`;
  const [camA,camB]=world.fighters,midX=(camA.x+camB.x)/2,midZ=(camA.z+camB.z)/2,spread=Math.hypot(camA.x-camB.x,(camA.z-camB.z)*.72);const aspect=innerWidth/innerHeight,baseHalfH=Math.max(8.5,12.8/aspect),targetHalfH=baseHalfH+Math.max(0,spread-7)*.26+Math.max(0,Math.abs(camA.z-camB.z)-3)*.22;cameraHalfH+=(targetHalfH-cameraHalfH)*Math.min(1,dt*2.2);camera.top=cameraHalfH;camera.bottom=-cameraHalfH;camera.left=-cameraHalfH*aspect;camera.right=cameraHalfH*aspect;camera.updateProjectionMatrix();const targetX=Math.max(-12,Math.min(12,midX)),targetZ=Math.max(-7,Math.min(7,midZ));camera.position.x+=(targetX-camera.position.x)*Math.min(1,dt*2.4);camera.position.z+=(25-targetZ*.18-camera.position.z)*Math.min(1,dt*2.4);camera.lookAt(targetX,1,targetZ);
}
